...
class Order {
    ...
    const phonesToOrdersValueSrting = orders.map(
        o =>
        {
            const arr = new Array(_.random(1, 4, false)).fill(null).map(() =>  phones[_.random(1, phones.length - 1)]);

            return [...new Set(arr)].map(p => `(${o.id}, ${p.id}, ${_.random(1, 3, false)})`).join(',')
        }).join(',');

    const res2 = await this._client.query(
        `INSERT INTO orders_to_products (order_id, product_id, quantity)
        VALUES ${phonesToOrdersValueSrting};`
    ) 
